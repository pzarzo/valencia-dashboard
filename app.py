...
def infer_result_series(df: pd.DataFrame) -> pd.Series:
    # Devuelve 'V', 'E', 'D' (o '?') según resultado_valencia o puntos
...
# Temporadas: opción "Todas" como primera; sin nada seleccionado por defecto
...
with tab_resumen:
    ...
    if len(df) > 0 and "puntos" in df.columns and "temporada" in df.columns:
        ...
        fig_pts = px.bar(
            by_temp_pts, x="temporada", y="Puntos",
    ...
    if len(df) > 0 and "temporada" in df.columns:
        ...
        fig_bar = px.bar(
            temp_res, x="temporada", y="Partidos", color="Res",
...
with tab_records:
    ...
    def show_match_record(title: str, row: dict):
        if not row:
            st.write(f"**{title}:** —")
            return
        ...
        rival = row.get("rival", "")
        gf = row.get("goles_valencia", "")
        gc = row.get("goles_rival", "")
        comp = row.get("competicion", None)
        reslab = row.get("resultado_valencia", None)

        parts = [f"**{title}:** {temporada} · {fecha} · {condicion} vs {rival} · Resultado: {gf}-{gc}"]
        extras = []
        if comp:
            extras.append(f"Competición: {comp}")
        if reslab:
            extras.append(f"Resultado: {reslab}")
        if extras:
    ...
    if df_rem is not None and "temporada" in df_rem.columns and len(df_rem) > 0:
    ...
    # Partido más temprano y tardío por hora
    if "hora" in df_rec.columns and df_rec["hora"].notna().any():
        ...
        if len(dfh) > 0:
            # idxmin/idxmax sobre strings de time puede ser sensible; convertimos a minutos
            def to_minutes(t):
                return t.hour * 60 + t.minute if t is not None else None
            dfh = dfh.assign(_mins=[to_minutes(t) for t in dfh["_hora"]])
            row_early = dfh.loc[dfh["_mins"].idxmin()].to_dict()
            show_match_record("Partido más temprano", row_early)
            row_late = dfh.loc[dfh["_mins"].idxmax()].to_dict()
    ...
    if "franja" in df_rec.columns and "puntos" in df_rec.columns and df_rec["franja"].notna().any():
        res_v = (pd.to_numeric(df_rec["puntos"], errors="coerce") == 3)
        t = df_rec.assign(V=res_v).groupby("franja").agg(PJ=("V", "count"), V=("V", "sum")).reset_index()
    ...
    if "resultado_valencia" in df_rec.columns:
        resd = df_rec["resultado_valencia"].astype(str).str.lower().str.startswith("der")
    elif "puntos" in df_rec.columns:
        resd = (pd.to_numeric(df_rec["puntos"], errors="coerce") == 0)
    if resd is not None and "temporada" in df_rec.columns:
        tmpd = df_rec.assign(D=resd).groupby("temporada")["D"].sum().reset_index(name="Derrotas")
        if len(tmpd) > 0:
            tmin = tmpd.loc[tmpd["Derrotas"].idxmax()]
    ...
    if "goles_valencia" in df_rec.columns and "goles_rival" in df_rec.columns:
        emp00 = df_rec[
            (pd.to_numeric(df_rec["goles_valencia"], errors="coerce") == 0) &
            (pd.to_numeric(df_rec["goles_rival"], errors="coerce") == 0)
        ]
